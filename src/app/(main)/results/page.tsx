...
interface Result {
  ...
  subject: string;
  subject_code: string; 
  marks: number; // ডেটাবেজের marks কলাম
  grade: string; // ডেটাবেজের grade কলাম
  cgpa: number;  // ডেটাবেজে জিপিএ পয়েন্টের কলামের নাম cgpa
  student_name: string;
}
...
export default function ResultsPage() {
  ...
  // Student: load own results | Admin: load departments
  useEffect(() => {
  ...
  const handleDeptSelect = async (dept: string) => {
    try {
      ...
      const { data } = await api.get(`/batches?department=${encodeURIComponent(dept)}`);
      ...
      console.error("Batch loading error in Results:", error);
      setBatches([]);
    }
  };
  ...
  // 🛠️ জিপিএ/সিজিপিএ ক্যালকুলেশন লজিক ফিক্স (ডেটাবেজ কলাম r.cgpa এর উপর ভিত্তি করে)
  const calcCGPA = (rows: Result[]) => {
    if (!rows || rows.length === 0) return '0.00';
    const totalPoints = rows.reduce((s, r) => s + Number(r.cgpa || 0), 0);
    return (totalPoints / rows.length).toFixed(2);
  };
                  ...
                  <tbody>
                    {rows.map(r => (
                      <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50">
                        ...
                        {/* 🛠️ ডেটাবেজে ক্রেডিট কলাম না থাকায় এখানে ডিফল্ট 3.00 বসালাম */}
                        <td className="px-3 py-3 text-center text-gray-600">3.00</td>
                        <td className="px-3 py-3 text-center">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${GRADE_COLORS[r.grade] || 'bg-gray-100 text-gray-600'}`}>
                            {r.grade}
                          </span>
                        </td>
                        {/* 🛠️ r.grade_point পরিবর্তন করে r.cgpa রিড করা হলো */}
                        <td className="px-3 py-3 text-center text-gray-600">{Number(r.cgpa).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
